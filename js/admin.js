...
function showConfirm(msg) {
  return new Promise(resolve => {
    ...
    modal.id = 'appConfirmModal'; modal.className = 'sale-details-modal';
    modal.innerHTML = `
...
function initAdmin() {
  ...
  btnHistory.addEventListener('click', () => { setActiveMenuButton(btnHistory); renderAdminHistory(); });
...
function openProductModal(existingId = null) {
  ...
  const modal = openFloatingModal(formHtml, (m) => {
    ...
    m.querySelector('#floatingProductForm').addEventListener('submit', async e => {
      e.preventDefault();
      ...
      const image       = document.getElementById('fpImage').value.trim() || './imagenes y recursos/default.jpg';
      ...
      const toSheets = (sid) => ({
        id: sid, nombre: name, categoria: category, precio: price,
        costo: cost, codigo: code, seguimientoInventario: tracking,
        stock, imagen: image, descripcion: description
      });

      if (id) {
        // Editar
        const p = products.find(p => p.id === Number(id));
        if (p) Object.assign(p, { name, category, price, cost, code, tracking, stock, image, description });
        try { await apiUpdate('productos', toSheets(Number(id))); showToast('Producto actualizado.'); }
        catch (err) { console.error(err); showToast('Error al sincronizar con Sheets.', 'error'); }
      } else {
        // Crear
        const newId = Date.now();
        products.push({ id: newId, name, category, price, cost, code, tracking, stock, image, description });
        try { await apiPost('productos', toSheets(newId)); showToast('Producto agregado.'); }
        catch (err) { console.error(err); showToast('Error al sincronizar con Sheets.', 'error'); }
      }
...
function renderProductsInMainArea() {
  ...
  if (products.length === 0) {
    ...
    document.getElementById('mainAddProduct').addEventListener('click', () => openProductModal());
    return;
  }
  ...
  historyContent.querySelectorAll('.main-edit').forEach(btn => {
    btn.addEventListener('click', e => openProductModal(Number(e.currentTarget.dataset.id)));
  });

  historyContent.querySelectorAll('.main-delete').forEach(btn => {
    btn.addEventListener('click', async e => {
      const id = Number(e.currentTarget.dataset.id);
      const ok = await showConfirm('¿Eliminar este producto? No se puede deshacer.');
...
async function deleteProduct(id) {
  ...
  try { await apiDelete('productos', { id }); showToast('Producto eliminado.'); }
  catch (e) { console.error(e); showToast('Eliminado localmente. Error en Sheets.', 'error'); }
  renderProducts(products);
  renderProductsInMainArea();
}
